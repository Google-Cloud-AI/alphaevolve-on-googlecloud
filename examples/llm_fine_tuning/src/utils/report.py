...
from ..evaluate import METRIC_NAME, SEED_BOOTSTRAP_SCORE
...
def _is_real_score(score: float) -> bool:
    return np.isfinite(score) and score > SEED_BOOTSTRAP_SCORE
...
def generate_report(
    ...
    # Figures
    _fig_evolution_progress(chrono_scores, seed_loss, save_dir)
...
def _print_summary(
    ...
    if seed_loss is not None:
        ...
        print("  Seed eval loss      : not evaluated (baseline skipped)")
    if best_loss is not None:
        print(f"  Best eval loss      : \033[32m{best_loss:.6f}\033[0m")
    else:
        print("  Best eval loss      : n/a (no successful programs)")
