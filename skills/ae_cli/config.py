...
def load_config(
  ...
  # Science Workbench: scope sessions to the Workbench project if unset. This
  # is a Workbench project UUID, not a GCP project, so it never sets `project`.
  wb_project_id = os.environ.get("WORKBENCH_PROJECT_ID", "").strip()
  if wb_project_id and not merged.get("user_pseudo_id"):
    merged["user_pseudo_id"] = wb_project_id
