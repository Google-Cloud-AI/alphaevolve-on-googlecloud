...
class AlphaEvolveClient:
    ...
    def acquire_programs(
        ...
        logger.info(
            f"Acquiring up to {desired_programs_count} programs from experiment: {experiment_name}"
        )
        request_data = {
    ...
    def submit_program_evaluations(
        ...
        logger.info(f"Submitting evaluations for experiment: {experiment_name}")
        request_data = {
            "parent": experiment_name,
            "evaluation_submissions": evaluation_submissions,
        }
        logger.info("Sending SubmitProgramsEvaluationsRequest:")
        return self._post_request(
            self.get_alpha_evolve_experiment_endpoint(experiment_name)
